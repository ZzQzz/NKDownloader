...
//
/*
 NKDownloadManager用法简介
 ---------------------------------------------------------------------------------------------
 基于UNSURLConnection封装的断点续传类，用于大文件下载，退出程序后，下次接着下载。
 
 -->1.在项目中导入NKDownloadManager.h头文件；
 -->2.搭建UI时，设置显示进度的UIProgressView的进度值:[[NKDownloadManager sharedManager] lastProgressWithUrl:url],
     这个方法的返回值是float类型的；
     设置显示文件大小/文件总大小的Label的文字：[[NKDownloadManager sharedManager]fileSize:url]；
 
 -->3.开始或恢复下载任务的方法：[NKDownloadManager sharedManager] downloadWithUrlString:(NSString *)urlString
 ...
 -->4.在下载出错的回调代码块中处理出错信息。在出错的回调代码块中或者暂停下载任务时，
      调用[[NKDownloadManager sharedManager] cancelDownloadTask:url]方法取消/暂停下载任务；
