...
//  DownloaderDemo
//
//  Created by NK on 15/9/23.
//  Copyright © 2015年 NK. All rights reserved.
//
 ...
 */
#define kNKDwonloadMaxTaskCount 2

static NKDownloadManager *mgr=nil;
...
-(instancetype)init
{
    if(self=[super init])
    {
        ...
        //注册系统内存不足的通知
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(systemSpaceInsufficient:) name:NKInsufficientSystemSpaceNotification object:nil];
        //注册程序下载完成的通知
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(downloadTaskDidFinishDownloading:) name:NKDownloadTaskDidFinishDownloadingNotification object:nil];
        //注册程序即将失去焦点的通知
...
-(void)systemSpaceInsufficient:(NSNotification *)sender{
    ...
    [[NKDownloadManager shredManager] cancelDownloadTask:urlString];
}
/**
...
-(void)downloadTaskWillBeTerminate:(NSNotification *)sender{
    ...
    [[NKDownloadManager shredManager] cancelAllTasks];
}
/**
 *  下载完成通知调用的方法
 *
...
-(void)downloadTaskDidFinishDownloading:(NSNotification *)sender{
    ...
    if(_taskDict.count<kNKDwonloadMaxTaskCount){
...
+(instancetype)shredManager
{
    ...
    dispatch_once(&onceToken, ^{
        mgr=[[NKDownloadManager alloc]init];
    });
    return mgr;
}
-(void)downloadWithUrlString:(NSString *)urlString toPath:(NSString *)destinationPath process:(ProcessHandle)process completion:(CompletionHandle)completion failure:(FailureHandle)failure
{
    ...
    NKDownloader *downloader=[NKDownloader downloader];
    @synchronized (self) {
...
{
    NKDownloader *downloader=[_taskDict objectForKey:url];
    [downloader cancel];
    @synchronized (self) {
...
{
    return [NKDownloader lastProgress:url];
}
-(NSString *)filesSize:(NSString *)url
{
    return [NKDownloader filesSize:url];
}
-(void)dealloc{
