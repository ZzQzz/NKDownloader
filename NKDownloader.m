...
//  大文件下载(断点续传)
//
//  Created by NK on 15/9/21.
//  Copyright (c) 2015年 NK. All rights reserved.
//

#import "NKDownloader.h"
#import <UIKit/UIKit.h>

@implementation NKDownloader
{
...
-(void)connection:(NSURLConnection *)connection didReceiveData:(NSData *)data
{
    ...
    if(freeSpace<1024*1024*20){
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:NKInsufficientSystemSpaceNotification object:nil userInfo:@{@"urlString":_url_string}];
        return;
    }
    ...
    NSString *sizeString=[NKDownloader filesSize:_url_string];
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:NKProgressDidChangeNotificaiton object:nil userInfo:userInfo];
    
    //计算网速
    NSString *speedString=@"0.00Kb/s";
    NSString *growString=[NKDownloader convertSize:_growth*(1.0/0.1)];
...
{
    [[NSNotificationCenter defaultCenter] postNotificationName:NKDownloadTaskDidFinishDownloadingNotification object:nil userInfo:@{@"urlString":_url_string}];
    if(_completion)
        _completion();
}
/**
